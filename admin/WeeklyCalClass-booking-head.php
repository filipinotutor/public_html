...
class WeeklyCalClassHead {
	...
	*/
	// end database ---------------------------------------------
    function WeeklyCalClassHead ($day, $month, $year, $user_id) {
...
//      end hourly                                 TABLE -----------------------------------------------------------------------------------------------------------------------	
    ...
    $Output.='<ul id="calendarhead">';
	
   // $Output .= "<li ><a href='".$PHP_SELF."?day=".$ant."&month=".$monthAnt."&year=".$yearAnt."'>&laquo; previous week</a></li>";
	$Output .= '<li id="21-5-2013">&laquo; previous week</li>';
	//$Output .= '<li id="'.$ant."-".$monthAnt."-".$yearAnt.'">&laquo; previous week</li>';
    $Output .= "<li class='active'>".$week['start'].'-'.$week['end'] .','.$year."</a></li> ";
    $Output .= '<li id="'.$post."-".$monthPost."-".$yearPost.'">next week &raquo;</li>';
	$Output .= "</ul>";
	$Output .="<button style='margin-top:0px;' class='btn btn-primary btn-large navbar-form pull-right' type='submit' name='send' value='Save' ><i class='icon-ok-sign icon-white'></i> Update</button>";
	$Output .="</div></div></div>";
