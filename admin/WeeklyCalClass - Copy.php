...
class WeeklyCalClass {
	...
	function get_sched($time, $day, $month, $year) {
		$query_get = "SELECT status FROM schedules WHERE time = '$time' AND day = '$day' AND month = '$month' AND year = '$year'";
    ...
    function showCalendar () {
        ...
        $Output .= $this->WeekTable ($this->date ["daymonth"], $this->date ["dayWeek"], $this->date["numdaysmonth"], $this->date["monthName"], $this->day, $this->month, $this->year);
        $Output .= "</table>";
    ...
    function WeekTable ($daymonth, $dayWeek, $numdaysmonth, $monthName, $day, $month, $year) {
		...
		//	print "$i:$v";
        {
			...
			foreach ($min as $v)
			{
				...
				for ($n=0; $n<=6; $n++)
				{
					...
					$got=$this->get_sched($outtime, $dayLink[$n]["day"], $dayLink[$n]["month"], $dayLink[$n]["year"]);
...
//      end hourly                                 TABLE -----------------------------------------------------------------------------------------------------------------------	
    ...
    $Output .= "<li class='active'><a href=''>" .$week['start'].'-'.$week['end'] .','.$year."</a></li> ";
    $Output.="<li ><a href='".$PHP_SELF."?day=".$post."&month=".$monthPost."&year=".$yearPost."'>next week &raquo;</a></li>
    
	</ul>
	<button style='margin-top:0px;' class='btn btn-primary btn-large navbar-form pull-right' type='submit' name='send' value='Save' ><i class='icon-ok-sign icon-white'></i> Update</button>
    </div></div></div>";
