				...
				foreach($hoursselected as $selected)
					{
						...
						//echo ' hour:'.$hour.' day:'.$day. ' year:'.$year. ' sched_id:'.$sched_id.'<br />'; 
						//////// check credits before scheduling----------------------
						
						//check selected schedule if exists
						$matched = $page_protect->get_sched_matched($hour, $day, $month, $year, $page_protect->user_id);
						if(!$matched) //duplicate schedule
						{
							...
							if( $credit_value-1 >= 0)
							{
								...
								if($res)
								{
									...
									//bind schedule and credit so we will know which credit was used when booking the schedule
									//echo '$sched_id='.$sched_id;
									//echo '$credit_id='.$credit_id;
									
									$page_protect->insert_credit_usage($credit_id, $sched_id, $status='booked');
									
									if(intval($credit_value) > 1)//if more than 1
									{
										$credit_value=$credit_value-1;
										$page_protect->update_student_credit($credit_id, $credit_value, 1);
									}	
									else if($credit_value == 1) //if 1
									{
										$credit_value=$credit_value-1;
