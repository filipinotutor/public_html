				...
				<div id="4B" class="tab-pane fade in <?php echo $tab=='4B' ? 'active' : '';?> col-xs-12 col-md-12">
		          ...
		          <div class="row">
					<div class="col-md-12">
						...
						<tr>
						<th class="hidden-xs" style="width:10px;">Order</th>
						...
						<?php
								...
								if(isset($_POST['category']) != "" && isset($_POST['submit_filter'])){
									$cat_choice = explode(":",$_POST['category']);
									if($cat_choice[1]==0){
										$cat_choice[1]=NULL;
									}
									if(isset($_POST['filter']) != "" && isset($_POST['submit_filter'])){
										$filter = " ORDER BY ".$_POST['filter']." ASC";
										$res = $page_protect->get_materials($cat_choice[0],$cat_choice[1],$filter);
									}else{
										$res = $page_protect->get_materials($cat_choice[0],$cat_choice[1],NULL);
									}
								}else{
									if(isset($_GET['c'])){
										$cat_choice = explode(":",$_GET['c']);
										if($cat_choice[1]==0){
										$cat_choice[1]=NULL;
										}
									}else{
								...
								if($row[7] != '0'){
								echo '<td>
										<a href="../includes/materials_content.php?mid='.$row[0].'&t=4B" target="blank" class="btn btn-primary">
