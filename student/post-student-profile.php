    ...
    */
    if(!empty($value)) {
        ...
        */
        ...
        //print_r($_POST);
		if(!$page_protect->has_profile_student($page_protect->user_id)) //not found in students table, insert
		{
			$insert=true;
		}
			...
			switch ($name)
			{
					...
					case "birthday";
							...
							if($insert){
								$page_protect->insert_profile_student($pk, $name, $value); //not found, insert	
								}
							else{
								$page_protect->update_student_profile_ajax($pk, $name, $value);
								}	
							break;
